...
class MeasurementData():
    ...
    def __init__(self, measurement_data: rf.Network, f_c, L, l_p1, l_p2):
        """
        ...
        self.f_c = f_c  # cutoff frequency in Hz
        self.L = L      # length of the sample in m
        self.lam_c = c_const / self.f_c

        self.l_p1 = l_p1  # position of the sample from port1 in m
        self.l_p2 = l_p2  # position of the sample from port2 in m
        self.wg_length = self.l_p1 + self.l_p2 + self.L  # total length of the waveguide including the sample in m
        ...
        # self.S11  = self.s_params['S11'].values
        # self.S21  = self.s_params['S21'].values
        # self.S12  = self.s_params['S12'].values
        # self.S22  = self.s_params['S22'].values

        # self.S11dB = self.s_params['S11dB'].values
        # self.S21dB = self.s_params['S21dB'].values
        # self.S12dB = self.s_params['S12dB'].values
        # self.S22dB = self.s_params['S22dB'].values

        # self.f = self.s_params['freq'].values  # frequency in Hz
    ...
    # @s_params.setter
    # def s_params(self, value):
    #     """
    #     Sets the S-parameters DataFrame.
    #     """
    #     if isinstance(value, pd.DataFrame):
    #         self._s_params = value
    #     else:
    #         raise ValueError("S-parameters must be a pandas DataFrame.")
    ...
    def _deembedd(self, l_p1=0, l_p2=0):
        ...
        """
        if l_p1 < 0 or l_p1 > 0:
            self.logger.info(f"De-embedding S-parameters with sample length L = {self.L}, "
                             f"Port 1 position = {l_p1} Port 2 position = {l_p2} m. "
                             f"Total measurement length L={self.wg_length} m")
            self.logger.debug(self.wg_drawing(self.L, l_p1, l_p2))
            
            self.S11 = self.s_params.apply(
                lambda row: row['S11'] * np.power(np.exp(self.gamma(row['freq'], self.lam_c, epsr=1, mur=1) * l_p1), 2), 
                axis=1)
            self.logger.info(f"De-embedded S11: {self.rect2pol(self.s_params['S11'].values)}")
        else:
        ...
        if (l_p1 < 0 or l_p1 > 0) and (l_p2 < 0 or l_p2 > 0):
            self.S21 = self.s_params.apply(
                lambda row: row['S21'].conjugate() * 
                    np.exp(self.gamma(row['freq'], self.lam_c, epsr=1, mur=1) * l_p1)*
                    np.exp(self.gamma(row['freq'], self.lam_c, epsr=1, mur=1) * l_p2)
                ,  axis=1)
            self.logger.info(f"De-embedded S21: {self.rect2pol(self.s_params['S21'].values)}")
            self.logger.debug(f"De-embedded S-parameters DataFrame:\n{self.s_params}")
        else:
            # conjungate S21 if l_p1 or l_p2 is not set
            self.S21 = -self.s_params['S21'].values.conjugate()
